...
import (
	...
	"github.com/cage1016/ms-sample/internal/app/count/endpoints"
	"github.com/cage1016/ms-sample/internal/app/count/service"
	"github.com/cage1016/ms-sample/internal/pkg/errors"
	pb "github.com/cage1016/ms-sample/pb/count"
)
...
func MakeGRPCServer(endpoints endpoints.Endpoints, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) (req pb.CountServer) { // Zipkin GRPC Server Trace can either be instantiated per gRPC method with a
	...
	return &grpcServer{
		tic: grpctransport.NewServer(
			...
			append(options, grpctransport.ServerBefore(opentracing.GRPCToContext(otTracer, "Tic", logger), kitjwt.GRPCToContext()))...,
		),
		...
		tac: grpctransport.NewServer(
			...
			append(options, grpctransport.ServerBefore(opentracing.GRPCToContext(otTracer, "Tac", logger), kitjwt.GRPCToContext()))...,
		),
	}
}
...
func NewGRPCClient(conn *grpc.ClientConn, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) service.CountService { // Zipkin GRPC Client Trace can either be instantiated per gRPC method with a
