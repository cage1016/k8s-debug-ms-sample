...
func TicHandler(m *bone.Mux, endpoints endpoints.Endpoints, options []httptransport.ServerOption, otTracer stdopentracing.Tracer, logger log.Logger) {
	m.Post("/tic", httptransport.NewServer(
		...
		append(options, httptransport.ServerBefore(opentracing.HTTPToContext(otTracer, "Tic", logger), kitjwt.HTTPToContext()))...,
	))
}
...
func TacHandler(m *bone.Mux, endpoints endpoints.Endpoints, options []httptransport.ServerOption, otTracer stdopentracing.Tracer, logger log.Logger) {
	m.Get("/tac", httptransport.NewServer(
		...
		responses.EncodeJSONResponse,
		append(options, httptransport.ServerBefore(opentracing.HTTPToContext(otTracer, "Tac", logger), kitjwt.HTTPToContext()))...,
	))
}
...
func NewHTTPClient(instance string, otTracer stdopentracing.Tracer, zipkinTracer *stdzipkin.Tracer, logger log.Logger) (service.CountService, error) { // Quickly sanitize the instance string.
